...
import { TITLE_BAR_ACTIVE_FOREGROUND, TITLE_BAR_INACTIVE_FOREGROUND, WORKBENCH_BACKGROUND } from '../../../common/theme.js';
...
export class BrowserTitlebarPart extends Part implements ITitlebarPart {
	...
	override updateStyles(): void {
		...
		if (this.element) {
			...
			// Cardified workbench: the title bar always paints with the
			// workbench background so it reads as one continuous "void"
			// surface with the gap area and the activity bar. Using
			// WORKBENCH_BACKGROUND here (rather than TITLE_BAR_*_BACKGROUND)
			// also makes the OS-painted window controls overlay (close /
			// maximize / minimize on Windows) match, because
			// electron-browser/titlebarPart.ts passes this inline background
			// to `setTitleBarOverlay({ color })`.
			const titleBackground = WORKBENCH_BACKGROUND(this.theme).toString();
			...
			// Title bar bottom border suppressed: in the cardified workbench the
			// title bar reads as part of the unified void background, and a
			// 1px stroke at its bottom edge breaks that continuity.
